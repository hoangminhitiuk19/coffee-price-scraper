...
def capture_element_screenshot():
    ...
    with sync_playwright() as p:
        ...
        # Step 5: Wait for the target element to load
        target_selector = "#gia-noi-dia"  # Replace with the correct selector
        page.wait_for_selector(target_selector, timeout=60000)

        # Step 6: Capture screenshot of the target element
        page.locator(target_selector).screenshot(path=screenshot_path)

        # Confirm screenshot saved
        print(f"Element screenshot saved at {screenshot_path}")
...
def extract_text(image):
    ...
    reader = easyocr.Reader(['vi'])  # Include relevant languages
...
def main():
    # Step 1: Capture screenshot of a specific element
